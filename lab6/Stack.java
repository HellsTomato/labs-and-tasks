...
public class Stack<T> {
    ...
    // Создаёт массив data указанного размера (capacity - максимальный размер стека).
    public Stack(int capacity) {
        data = (T[]) new Object[capacity];
        // изначально стек пустой, поэтому 0
        size = 0;
    }

    // Метод добавления элемента в стек
    public void push(T element) {
        // проверка, если массив заполнен (size равно его длине), выбрасываем исключение
        if (size == data.length) {
            throw new StackOverflowError("Стек переполнен");
        }
        // помещаем элемент в массив по индексу size (увеличиваем на 1 после добавления)
        data[size++] = element;
    }

    // Метод удаления элемента из стека
    public T pop() {
        // проверка, если стек пуст, выбрасывается исключение
        if (size == 0) {
    ...
    // pop() не всегда безопасно, если вы не уверены, что хотите удалять элемент
    public T peek() {
        // проверка, если стек пуст, выбрасывается исключение.
        if (size == 0) {
            throw new IllegalStateException("Стек пуст");
        }
        // Возвращаем элемент из массива по индексу size - 1 (верхний элемент).
        return data[size - 1];
    }
